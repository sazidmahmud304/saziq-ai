...
const STRIPE_YEARLY_LINK  = "https://buy.stripe.com/YOUR_YEARLY_LINK";
...
function PaywallModal({ C, isMobile, onClose, onUpgrade, user }) {
  ...
  const [activateEmail, setActivateEmail] = useState(user?.email || "");
  ...
  const handleUpgrade = (planKey) => {
    ...
    // After payment, user comes back and enters their email to activate Pro
    window.open(link, "_blank");
    setShowActivate(true);
    setActivatePlan(planKey);
  };
      ...
      <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 18, width: "100%", maxWidth: 720, maxHeight: "90vh", overflowY: "auto", animation: "fadeUp .3s ease" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding: "20px 24px", display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: 12 }}>
          {PLANS_LIST.map(plan => (
           ...
           {/* ── Activation panel — shown after Stripe redirect ── */}
        {showActivate && !activateSuccess && (
          <div style={{ margin: "0 24px 20px", background: `${C.acc}10`, border: `1px solid ${C.acc}30`, borderRadius: 12, padding: "18px 20px" }}>
            <div style={{ fontSize: 14, fontWeight: 800, color: C.txt, marginBottom: 4 }}>✅ Paid? Activate your Pro account</div>
            <div style={{ fontSize: 12.5, color: C.txtS, marginBottom: 14 }}>After completing payment on Stripe, enter the email you used to pay and click Activate.</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              <input
                value={activateEmail}
                onChange={e => setActivateEmail(e.target.value)}
                placeholder="Email you used to pay on Stripe"
                style={{ background: C.inputBg, border: `1px solid ${C.border}`, borderRadius: 8, padding: "10px 13px", color: C.inputTxt, fontSize: 13.5, width: "100%", caretColor: C.acc }}
                onFocus={e => e.target.style.borderColor = C.acc}
                onBlur={e => e.target.style.borderColor = C.border}
              />
              {activateErr && <div style={{ fontSize: 12, color: C.red }}>{activateErr}</div>}
              <button
                onClick={() => {
                  // Verify: the email they enter must match their logged-in account email
                  // In production you would verify via Stripe webhook. For now we check email match.
                  if (!activateEmail.trim()) { setActivateErr("Please enter your email."); return; }
                  if (user?.email && activateEmail.trim().toLowerCase() !== user.email.toLowerCase()) {
                    setActivateErr("Email does not match your account. Make sure you paid with: " + user?.email);
                    return;
                  }
                  // Activate!
                  ...
                  setActivateErr("");
                  setTimeout(() => {
                    onUpgrade(activatePlan || "pro");
                  }, 1500);
                }}
                style={{ background: `linear-gradient(135deg,${C.acc},${C.acc2})`, border: "none", borderRadius: 9, padding: "11px", color: "#fff", fontWeight: 700, fontSize: 14, cursor: "pointer", boxShadow: `0 3px 14px ${C.acc}40` }}
              >
                ⚡ Activate Pro Access
              </button>
            </div>
        ...
        {activateSuccess && (
          <div style={{ margin: "0 24px 20px", background: `${C.green}15`, border: `1px solid ${C.green}40`, borderRadius: 12, padding: "18px 20px", textAlign: "center" }}>
            <div style={{ fontSize: 24, marginBottom: 6 }}>🎉</div>
            <div style={{ fontSize: 15, fontWeight: 800, color: C.green }}>Pro Access Activated!</div>
            <div style={{ fontSize: 12.5, color: C.txtS, marginTop: 4 }}>Enjoy unlimited analyses. Closing in a moment…</div>
          </div>
        )}
