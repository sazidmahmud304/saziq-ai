...
// ── Google OAuth — SDK loaded in index.html ────────────────────────────────
...
async function callAI(system, messages) {
  // Always calls /api/analyze — the Vercel serverless function that holds the API key.
  // This works on Vercel (production) and also locally if you run `vercel dev`.
...
// SDK is loaded in index.html so it's available immediately
function GoogleSignInButton({ C, onAuth }) {
  ...
  useEffect(() => {
    // Wait for SDK to be ready (it loads async from index.html)
    const tryInit = () => {
      ...
      } catch(e) { setErr("Google login error: " + e.message); }
    };
    tryInit();
  }, []);
  ...
  if (GOOGLE_CLIENT_ID === "YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com") {
    return (
      <div style={{ border: `1px solid ${C.border}`, borderRadius: 10, padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "center", gap: 10, opacity: 0.45, background: C.card }}>
        ...
        <span style={{ fontSize: 13.5, color: C.txtS }}>Google Login (setup required)</span>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {err && <div style={{ fontSize: 12, color: C.red, marginTop: 6, textAlign: "center" }}>⚠ {err}</div>}
      {!ready && !err && GOOGLE_CLIENT_ID !== "YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com" && (
        <div style={{ textAlign: "center", fontSize: 12, color: C.txtS, padding: "10px" }}>Loading Google Sign-In…</div>
      )}
    </div>
  );
}
...
// === PAYWALL MODAL (Razorpay — Indian Payment Gateway) =======================
function PaywallModal({ C, isMobile, onClose, onUpgrade, user }) {
  ...
  const PLANS_LIST = [
    ...
    {
      key: "pro", name: "Pro",
      price: billing === "monthly" ? "₹749" : "₹499",
      period: billing === "monthly" ? "/month" : "/month (₹5999/year)",
      badge: "MOST POPULAR",
      features: [
      ...
      monthly: PRICE_PRO_MONTHLY, yearly: PRICE_PRO_YEARLY,
    },
    {
      key: "team", name: "Team",
      price: billing === "monthly" ? "₹1499" : "₹999",
      period: billing === "monthly" ? "/month" : "/month (₹11999/year)",
      badge: "BEST VALUE",
      features: [
        "Everything in Pro",
        "Up to 5 users",
      ...
      monthly: PRICE_TEAM_MONTHLY, yearly: PRICE_TEAM_YEARLY,
    },
  ];
  ...
  const handleUpgrade = (plan) => {
    ...
    if (RAZORPAY_KEY_ID === "rzp_live_YOUR_KEY_ID") {
      ...
      setErr("⚠ Razorpay not set up yet. Add your Key ID to App.jsx to enable payments.");
      setLoading(null);
      return;
    }
    ...
    loadRazorpay(() => {
      const options = {
        ...
        image: "https://via.placeholder.com/60x60/0EA5E9/white?text=S",
        prefill: {
          email: user?.email || "",
          name: user?.name || "",
        },
        ...
        handler: function(response) {
          ...
          setSuccess({ plan: plan.key, paymentId: response.razorpay_payment_id });
          setLoading(null);
          // Activate plan after 2 seconds
          setTimeout(() => {
            onUpgrade(plan.key);
          }, 2000);
        },
      ...
      try {
        ...
        rzp.on("payment.failed", (response) => {
          setErr("Payment failed: " + (response.error?.description || "Unknown error. Please try again."));
          setLoading(null);
        });
        rzp.open();
      } catch(e) {
        setErr("Could not open payment. Check your Razorpay Key ID.");
        setLoading(null);
      }
    });
  };
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:300, background:"#00000085", display:"flex", alignItems:"center", justifyContent:"center", padding:16 }} onClick={onClose}>
      <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:18, width:"100%", maxWidth:720, maxHeight:"92vh", overflowY:"auto", animation:"fadeUp .3s ease" }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ padding:"22px 24px 16px", textAlign:"center", borderBottom:`1px solid ${C.border}`, position:"relative" }}>
          ...
          <div style={{ fontSize:20, fontWeight:900, color:C.txt, marginBottom:4 }}>
            {success ? "🎉 Payment Successful!" : `You've used all ${FREE_LIMIT} free analyses today`}
          </div>
          <div style={{ fontSize:13.5, color:C.txtS }}>
            {success ? `${success.plan.charAt(0).toUpperCase()+success.plan.slice(1)} plan activated! Enjoy unlimited access.` : "Upgrade for unlimited access. Pay securely with UPI, Cards, Net Banking."}
          </div>
          {!success && (
            <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, marginTop:14 }}>
              ...
              <span style={{ fontSize:12.5, color:billing==="yearly"?C.acc:C.txtS, fontWeight:billing==="yearly"?700:400 }}>
                Yearly <span style={{ fontSize:10, background:`${C.green}20`, color:C.green, border:`1px solid ${C.green}30`, borderRadius:4, padding:"1px 6px", fontFamily:"monospace" }}>SAVE 33%</span>
              </span>
            </div>
          )}
        </div>
        ...
        {success ? (
          <div style={{ padding:"40px 24px", textAlign:"center" }}>
            ...
            <div style={{ fontSize:16, fontWeight:800, color:C.green, marginBottom:8 }}>Pro Access Activated!</div>
            <div style={{ fontSize:13, color:C.txtS, marginBottom:8 }}>Payment ID: <code style={{ fontFamily:"monospace", color:C.acc }}>{success.paymentId}</code></div>
            <div style={{ fontSize:12.5, color:C.txtS }}>Keep this ID for your records. Closing automatically…</div>
          </div>
        ) : (
          <>
            {/* Plans grid */}
            <div style={{ padding:"20px 24px", display:"grid", gridTemplateColumns:isMobile?"1fr":"repeat(3,1fr)", gap:12 }}>
              {PLANS_LIST.map(plan => (
                <div key={plan.key} style={{ background:plan.key==="pro"?`${C.acc}10`:C.card2, border:`2px solid ${plan.key==="pro"?C.acc:plan.key==="team"?C.purple:C.border}`, borderRadius:14, padding:"18px 16px", position:"relative", display:"flex", flexDirection:"column" }}>
                  {plan.badge && <div style={{ position:"absolute", top:-10, left:"50%", transform:"translateX(-50%)", background:plan.key==="pro"?C.acc:C.purple, color:"#fff", fontSize:9, fontWeight:800, letterSpacing:1, padding:"3px 10px", borderRadius:10, fontFamily:"monospace", whiteSpace:"nowrap" }}>{plan.badge}</div>}
                  <div style={{ fontSize:14, fontWeight:800, color:plan.disabled?C.txtS:plan.color, marginBottom:4 }}>{plan.name}</div>
                  <div style={{ display:"flex", alignItems:"baseline", gap:2, marginBottom:4 }}>
                    <span style={{ fontSize:28, fontWeight:900, color:C.txt }}>{plan.price}</span>
                    <span style={{ fontSize:10, color:C.txtS, fontFamily:"monospace" }}>{plan.period}</span>
                  </div>
                  <div style={{ flex:1, display:"flex", flexDirection:"column", gap:7, marginBottom:14, marginTop:8 }}>
                    {plan.features.map(f => (
                      <div key={f} style={{ display:"flex", gap:7, alignItems:"flex-start", fontSize:12 }}>
                        <span style={{ color:plan.disabled?C.txtS:plan.color, fontSize:11, flexShrink:0, marginTop:1 }}>✓</span>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border:plan.disabled?`1px solid ${C.border}`:"none",
                      background:plan.disabled?"transparent":loading===plan.key?C.card2:`linear-gradient(135deg,${plan.color},${plan.color}BB)`,
                      color:plan.disabled||loading===plan.key?C.txtS:"#fff",
                      fontWeight:700, fontSize:13,
                      cursor:plan.disabled||loading===plan.key?"not-allowed":"pointer",
                      display:"flex", alignItems:"center", justifyContent:"center", gap:7,
                      boxShadow:plan.disabled||loading===plan.key?"none":`0 3px 14px ${plan.color}40`,
                    }}
                  >
                    {loading===plan.key ? <><span style={{width:14,height:14,border:"2px solid #ffffff40",borderTopColor:"#fff",borderRadius:"50%",animation:"spin .7s linear infinite",display:"inline-block"}}/> Opening…</> : plan.cta}
                  </button>
                </div>
              ))}
            </div>
            ...
            {/* Error message */}
            {err && <div style={{ margin:"0 24px 16px", background:`${C.red}12`, border:`1px solid ${C.red}30`, borderRadius:9, padding:"11px 15px", color:C.red, fontSize:13 }}>{err}</div>}

            {/* Payment methods badge */}
            <div style={{ margin:"0 24px", padding:"12px 16px", background:C.card2, border:`1px solid ${C.border}`, borderRadius:10, display:"flex", alignItems:"center", gap:10, flexWrap:"wrap", marginBottom:16 }}>
              <span style={{ fontSize:11.5, color:C.txtS, fontFamily:"monospace" }}>ACCEPTED:</span>
              {["📱 UPI", "💳 Cards", "🏦 Net Banking", "👛 Wallets"].map(m => (
                <span key={m} style={{ fontSize:12, color:C.txt, background:C.card, border:`1px solid ${C.border}`, borderRadius:6, padding:"3px 10px", fontWeight:500 }}>{m}</span>
              ))}
            </div>

            {/* Guest notice */}
            {user?.isGuest && (
              <div style={{ margin:"0 24px 16px", padding:"10px 14px", background:`${C.amber}12`, border:`1px solid ${C.amber}30`, borderRadius:9, textAlign:"center" }}>
                <div style={{ fontSize:12.5, color:C.amber, fontWeight:600 }}>⚠ You are browsing as Guest</div>
                <div style={{ fontSize:11.5, color:C.txtS, marginTop:3 }}>Please create a free account first, then upgrade to keep Pro permanently.</div>
              </div>
            )}
          </>
        )}
        ...
        <div style={{ padding:"12px 24px 20px", textAlign:"center", fontSize:11.5, color:C.txtD, fontFamily:"monospace", borderTop:`1px solid ${C.border}` }}>
          Powered by Razorpay · Secure Indian Payment Gateway · Cancel anytime
        </div>
      </div>
    </div>
  );
